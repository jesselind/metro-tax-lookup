...
- **IN PROGRESS (sky):** When we are still working to obtain or ship data and have **not** confirmed a county failure, use sky **IN PROGRESS** chrome (`InProgressBadge` / `InProgressCallout` / `InfoHintPopover` `variant="in-progress"`). Assessed-value chip badge label is **Coming soon** (Douglas `priorYearValuesInProgress` today). Do not list these in the COUNTY DATA GAP hub. Same maintainer doc: `docs/county-service-gap-callouts.md`.
