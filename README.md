...
3. Rebuild the district contact bundle (DOLA LG export, filtered to LGIDs in levy stacks):
   ...
   Reads `supporting-data/dola/lg-export-all.csv` (or pass `--lg-csv`) and `public/data/arapahoe-levy-stacks-by-tag-id.json`. Writes `public/data/colorado-special-district-directory.json`. Run after `build:ingest:ship` (or emergency `build:arapahoe-index`) when levy stacks change, or when you refresh the LG CSV.
