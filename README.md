...
Hand-curated **Who authorized this?** trail (**authority chain**) for selected stack rows (Cherry Creek `0501`, Littleton `0601`, Arapahoe County `2998`, and Sky Ranch Metro District No. 3 `4571`). JSON version 2 holds **structured facts only** plus explicit `family` (`school` | `county` | `metro`); resident wording is built from `src/content/levyAuthorityChainTemplates.ts` (master trail + family packs) via `src/lib/levyAuthorityChainBuild.ts`. The master trail stays across families, while validation follows the authority type: school/county measures require certified vote totals; metro measures require either published totals or a cited official `approval` record (folded onto each measure step). Metro `ballotIssue` is optional because district-run elections may not publish a county-style letter. Never add placeholder vote totals or an invented ballot label. Optional budget facts use `budget`; entry-specific recipient facts use `authority.whoGetsBody` / `whoGetsFacts`. **Metro mill numbers** are not hand-authored: Change from last year and optional Most notable change are derived from the same AUTH mills series as the modal history chart (`arapahoe-authority-mills-by-tax-year.json` via `authorityMillsChangeBlocks.ts`). School/county still author current/prior mills cites. Sourced facts use `https` URLs; reuse of the same PDF URL is fine when link text differs. Missing school/county ballot wording uses `no-stable-ballot-text`; metro uses `metro-election-records-unlocated` plus the best official district record available. The Spanish/AI ladder remains a one-off county fallback described in `docs/levy-explainer-authoring.md`. Durable missing-document hunts live in `src/content/authorityChainUnlocatedSources.ts`. Validate with `npm run validate:levy-authority-chain` (also in `prebuild`). E2E (`e2e/authority-chain.spec.ts`) is authority-agnostic. Methodology: in-app `/sources` and `docs/levy-explainer-authoring.md`.
