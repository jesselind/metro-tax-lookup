...
| `supporting-data/douglas/` | Douglas County Assessor published text dumps + tax-district mill PDF (Phase 8 **Go**; Phase 9/9b/9c ingest on branch, including parcel-record MVP). Douglas JSON remains unpublished pending intentional publication. See **`docs/county-build-inputs.md`** and **`docs/county-ingest.md`** (Douglas JSON on disk). |
...
| Engine | npm | Output |
| --- | --- | --- |
| Ingest (v2, canonical) | `build:ingest`, `build:ingest:ship`, `diff:ingest`, `classify:ingest`, `test:ingest` | `_ingest-out/` (compare) or staging + atomic land into `public/data/` with `--ship` |
