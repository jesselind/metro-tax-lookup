...
- **Property details (home):** Locked report is **levy-first**. On `lg+`, a sticky left rail (`3/12`) holds permanent address chrome, **Switch account type** (when offered), and **Jump to a section** (`HomeDashboardSectionNav`) on a full-height slate-50 rail (footer-matched) so the main column stays primary; the main column (`9/12`) leads with **Where is your money going?** then Property details and the rest. Below `lg`, one sticky full-bleed slate strip: the Jump disclosure summary includes the one-line address so either row toggles the menu; I Own | I Rent (and Switch under it) sit in the main column below that strip, not above it. The sticky aside is a direct child of the tall locked-report flex/grid parent (a short wrapper kills sticky). No summary-tile cluster and no Value summary. Section nav is one list curated by Own|Rent|BPP and what is mounted (`buildHomeDashboardJumps` / `parcelRecordSubsectionPresence`): levy first; Property details parent plus flat subsection jumps; county compare and Feedback always; comps always-on for Own Real. Outside click/tap and Escape collapse the mobile TOC; scroll-spy sets `aria-current` from curated jump focus ids only. **Property details** (Own / BPP) is a full-width block: scalar rows in **`ParcelRecordPanel`**, then **Property classification**, then **`ParcelRecordExtendedSection`** / **`ParcelRecordCountyTables`**. Actual / Assessed **Changed**, Year over year openers, and prior-year gap / in-progress badges live on the Appraised and assessed values rows. **Est. property tax** glossary sits on the levy stack Total dollars row. When Main Parcel **TaxYear** and **AssessmentYear** differ, a calm note appears above the values table. Assessed school and local building/land splits follow state use (`src/lib/parcelAssessmentRates.ts`); personal-property accounts use the DPT personal-property schedule by year. Empty fields show **No data found** with a popover and prefilled mailto. After property details, **See how {County} displays your data** (`LevyCountyCompareSection`) offers county-hosted property / BPP / levy / search links. After Feedback on a locked levy-ready report, a single **Back to top**.
...
5. **Douglas metro purpose rows (Abstract Tax Rates)**
   ...
   - Stack **Total** carries **Est. property tax** glossary (`term-property-tax`) on the dollars row (honesty brief; face = stack under `levyStackDollars`). Mill levy total **Changed** is not shown on a summary chip (each levy tile keeps its own Changed). Appraised and assessed values rows open separate valuation-history dialogs and may show **Changed** plus **Prior years missing** / Coming soon (`CountyPriorYearValuesGapPopover` / `CountyPriorYearValuesInProgressPopover`). Jump to the levy stack via section nav (`Where is your money going?` → `#home-levy-stack-subheading` / `#home-levy-stack-tiles`). Levy stack intro: Select a mill levy tile for more details (`#home-levy-stack-intro`; first-mention `term-mill-levy` popover on mill levy).
