...
| `tools/ingest/` | Classifier (`classify.py`), reader (`reader.py`), writer (`writer.py`), compare (`compare.py`), build CLI (`build.py`). Does not write `public/data/` until the production rebuild switches (Phase 5). |
| `tools/ingest/mappings/` | County mapping files (`arapahoe.json`). Column names → shared field names. |

**Policy:** Point people at live county/state sources (`/sources`). Commit transforms under `public/data/`, the mart download stamp under `tools/`, and small **irreplaceable** build inputs (today: `supporting-data/dola/property-tax-entities-export.csv`). Do not commit large weekly dumps (mart, GIS, cert PDFs, LG directory CSV).
...
This is not a rebuild. Do not point `npm run build:arapahoe-index` at it.

### New ingest (comparison only, Phase 4)

`tools/ingest/mappings/arapahoe.json` maps Arapahoe Data Mart column names (`TAGId`, `Pin`, `TotalActual`, …) to shared field names (`taxAreaId`, `accountId`, `totalActual`, …). Shared reader and writer code has no Arapahoe column names; the mapping file is the only place they appear.

**Read:** `tools/ingest/reader.py` — `read_levy_stack_rows(csv_path, mapping)` and `read_account_rows(csv_path, mapping)` return intermediate records keyed by shared field names.

**Write:** `tools/ingest/writer.py` — `write_comparison_dir(out_dir, ...)` converts intermediate records to the app JSON shapes and writes them to a comparison directory (`supporting-data/_ingest-out/`, gitignored). Never writes to `public/data/`.

**Compare:** `tools/ingest/compare.py` — `npm run diff:ingest -- <dir-a> <dir-b>` diffs levy stacks and the account map (e.g. `public/data` vs `supporting-data/_ingest-out`). Excludes `snapshot` metadata and `dolaMatch`. Situs is compared only when both directories have that file. Other shipping JSON is ignored.

**Run new ingest:** `npm run build:ingest -- --mapping tools/ingest/mappings/arapahoe.json --tag-file <TAG CSV> --parcel-file <MAIN PARCEL CSV> --out-dir supporting-data/_ingest-out --bundled-as-of YYYY-MM-DD`. Production rebuild stays `npm run build:arapahoe-index`.

**Tests:** `npm run test:ingest-reader` (`test_ingest_reader.py`) — synthetic two-CSV county (invented Arapahoe headers and El Paso-like headers). `npm run test:ingest` runs classifier + reader tests together. No mart CSVs in CI.
