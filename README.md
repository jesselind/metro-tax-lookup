...
- **Account type control:** Shown only when several PINs share the situs. A salmon action **button** (not a summary tile) reads **Switch account type** with the current Real / business personal property kind underneath, and opens the in-dashboard switcher modal (levy-detail shell; whole-row hit targets; shared Real vs BPP glossary line under the heading). Single-account properties omit it. The post-search multi-match chooser (and its Parcel PIN or AIN entry) is unlocked-search only; the locked report never shows that PIN panel — switch accounts from the dashboard control instead.
