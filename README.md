...
Hand-curated **Who authorized this?** trail (**authority chain**) for selected stack rows (prototypes: Cherry Creek School Dist 5, AUTH `0501`; Littleton Public Schools Dist 6, AUTH `0601`). JSON version 2 holds **structured facts only**; resident wording is built from `src/content/levyAuthorityChainTemplates.ts` via `src/lib/levyAuthorityChainBuild.ts`. **Ideology:** always cite the next-best official source; when the ideal PDF is missing, link where we looked (for example that year's Past Elections File Library) so residents can see why, and use openGaps for honest limits. Sourced facts use `https` URLs (validator checks the scheme only). `ballotTextKind: "unavailable"` still requires a hub `ballotTextSource` plus openGap `no-stable-ballot-text`. Prefer stable official URLs; deep-link PDFs when verified (see `docs/levy-explainer-authoring.md`). Do not link Clarity ENR SPA URLs. Open gaps use predefined ids (`openGapIds`) with shared resident copy. Match keys share `src/lib/levyEntryMatch.ts` with levy explainers. Validate with `npm run validate:levy-authority-chain` (also in `prebuild`). E2E (`e2e/authority-chain.spec.ts`) is authority-agnostic. Narrative: `/sources`. Authoring: `docs/levy-explainer-authoring.md`.
