...
- **County service gap callouts:** When county-published exports or hosted files fail (not app errors), ship **both** a **home dashboard** surface (compact callout and/or COUNTY DATA GAP summary tile) **and** **`/sources`**: a bullet in **When county data fails** (jump links only) plus one matching **COUNTY DATA GAP** red box in **Your property tax bill** where that topic is explained (1:1). Full checklist: `docs/county-service-gap-callouts.md`.
