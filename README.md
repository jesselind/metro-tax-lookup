...
A small **Next.js** site for Arapahoe County residents: the **home page** (`/`) combines address → PIN lookup with a single **Your property tax bill** card: when your stack includes a metro district, a **metro district share** block sits above the levy tiles; below that are the levy breakdown (tiles; **Add tile** or load by PIN). Policy pages: sources, privacy, accessibility. Old URL `/metro-tax-lookup` redirects to `/`.
...
| --- | --- | --- |
| **Address + property tax bill** | `/` | Offline address → PIN (`arapahoe-situs-to-pins.json`), then **Your property tax bill**: levy stack from PIN (TAG lines), optional metro share above tiles when LG IDs match. Optional **Add levy lines without loading a PIN** opens the same card; use **Add tile** in the grid to copy rows from the county levy table. Editable tiles and table. |
