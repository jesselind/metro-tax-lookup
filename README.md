...
A small **Next.js** site for Arapahoe County residents: the **home page** (`/`) combines address → PIN lookup, the **property tax levy breakdown** (tiles; **Add tile** or load by PIN), and (when your levy stack includes a metro district) the **metro district tax share** card. Policy pages: sources, privacy, accessibility. Old URL `/metro-tax-lookup` redirects to `/`.
...
| **Metro district tax share** | `/` (home card) | Shown only when a levy line matches a metro district in `metro-levies-2025.json` (LG ID). Compares total mills to metro district mills; shows share of your property tax *rate* that goes to the metro district (operations + debt). Uses the **same total mills as the levy stack** (sum of lines). There is **no manual district picker** and **no separate mills field** on this card. |
