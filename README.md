...
| Browser e2e | `e2e/` + `npm run test:e2e` (Playwright) | Smoke + critical flows on Chromium, Firefox, and WebKit. Shared address helpers in `e2e/helpers/addressLookup.ts` (street field is a combobox). `e2e/metro-yoy.spec.ts` covers amber stack callout, Changed badge, percent YoY summary, expandable breakdown, `*` dollar footnote (no stack-level bill $). Assert UI contracts — not live scrapes or brittle dollar snapshots |
...
Hand-curated **Who authorized this?** trail (**authority chain**) for selected stack rows (prototypes: Cherry Creek School Dist 5, AUTH `0501`; Littleton Public Schools Dist 6, AUTH `0601`; Arapahoe County, AUTH `2998`). JSON version 2 holds **structured facts only** plus explicit `family` (`school` | `county`); resident wording is built from `src/content/levyAuthorityChainTemplates.ts` (master trail + family packs) via `src/lib/levyAuthorityChainBuild.ts`. Summary attribution is per-entry `summarySource.text` (trimmed at build; linked in the panel; no leading/trailing whitespace in JSON). County TABOR entries may set `authority.governmentBillName` (default `the county`) for retention copy and credit open gaps. Optional budget facts use the `budget` field. Sourced facts use `https` URLs (validator checks the scheme only). `ballotTextKind: "unavailable"` requires a hub `ballotTextSource`, openGap `no-stable-ballot-text`, **no** measure `detail` (vote-identity body only). When a Spanish sample is the only ballot wording we can link among currently published county files, use `ballotTextLanguage: "es"` + `ballotTextEnglishSource: "ai_translation"` + openGap `ballot-text-spanish-only-ai-translation` (English body disclosed as AI translation; frame as findability today, not that English never existed). Put other attributed substance in `supportingFacts` and/or `budget` when needed (hard-facts rule). Prefer stable official URLs; deep-link PDFs when verified. Do not link Clarity ENR SPA URLs. Open gaps use predefined ids (`openGapIds`); most share static copy, and `no-temporary-credit-mill-split` is filled from the entry's mills + TABOR measure. Entry-specific mills takeaways use optional `mills.stepBody` (county pack stays generic). Match keys share `src/lib/levyEntryMatch.ts` with levy explainers. Durable list of official documents we could not locate: `src/content/authorityChainUnlocatedSources.ts` (author notes: `docs/authority-chain-unlocated-sources.md`; open rows also appear in a `/sources` disclosure). Validate with `npm run validate:levy-authority-chain` (also in `prebuild`). E2E (`e2e/authority-chain.spec.ts`) is authority-agnostic. Methodology and next-best source guidance: in-app `/sources` and `docs/levy-explainer-authoring.md`.
