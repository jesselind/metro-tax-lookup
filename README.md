...
Hand-curated **Who authorized this?** trail for selected stack rows (Cherry Creek `0501`, Littleton `0601`, Arapahoe County `2998`, Sky Ranch Metro District No. 3 `4571`). JSON version 2 holds structured facts plus `family` (`school` | `county` | `metro`); resident wording is built from `src/content/levyAuthorityChainTemplates.ts` via `src/lib/levyAuthorityChainBuild.ts`. **Metro mill numbers** are not hand-authored: Change from last year and optional Most notable change come from the same AUTH mills series as the modal history chart (`arapahoe-authority-mills-by-tax-year.json` via `authorityMillsChangeBlocks.ts`). Validate with `npm run validate:levy-authority-chain` (also in `prebuild`). Authoring methodology (source ladders, ballot wording, E2E notes): `docs/levy-explainer-authoring.md` and in-app `/sources`.
