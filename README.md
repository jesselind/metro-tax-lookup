...
- Main route `/`: address-to-PIN lookup from bundled situs JSON (exact key, soft street-type cleanup, then fuzzy street names at the same house number, with typeahead / did-you-mean), plus PIN or AIN paste into the address or parcel-id field; then levy stack and metro share.
