...
- **Account type control:** Shown only when several PINs share the situs. A salmon action **button** (not a summary tile) reads **Switch account type** with the current Real / business personal property kind underneath, and opens the in-dashboard switcher modal (levy-detail shell). Single-account properties omit it. The post-search multi-match chooser is unchanged.
