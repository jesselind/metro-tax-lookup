...
- `supporting-data/`
  ...
  - `metro-levies-2026.json` – normalized levy data derived from the PDF above. Schema: `year`, `snapshot.bundledAsOf` (ISO date when the JSON was generated from the PDF; shown in the UI), `source`, `districts[]` (each with `districtId`, `countyId`, `lgid`, `subdistrict`, `name`, `type`, `levies[]`, `aggregates`: `opsMills`, `debtMills`, `otherMills`, `totalMills`, `audit`). Indexes: `byLgid`, `byName`. Generated by `tools/extract_metro_levies_2026.py`; do not hand-edit.
...
- DOLA distributes a **metro districts shapefile** (often named `dlmetro.zip` on download) that includes:
...
- One-time process:
  - Download the shapefile from DOLA (not checked into this repo).
...
1. Download the DOLA metro districts shapefile from DOLA (the archive is often named `dlmetro.zip`).
2. In QGIS:
