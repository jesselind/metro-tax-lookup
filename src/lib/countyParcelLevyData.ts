...
/**
 * County app JSON: types, fetch-time validators, and lazy loaders for levy stacks,
 * account map (pin-to-tag), and parcel-record shards.
 *
 * URLs come from {@link countyAccountMapUrl} and siblings in `countyDataPaths.ts`
 * (`{dataRoot}/{countyId}-*`). Situs index fetch lives in `situsIndexLookup.ts`.
 *
 * Phase 10 (5.2.0): canonical module name. Deprecated re-exports remain at
 * `arapahoeParcelLevyData.ts` for forks. Ship gate: `countyLoaderContract.test.ts`.
 */
...
export type CountyLevyStackLine = {
  code: string;
  authorityName: string;
  effectiveYear?: string | null;
  status?: string | null;
  dolaMatch: CountyDolaMatch;
};

export type CountyLevyStack = {
  tagId: string;
  taxYear?: string | null;
  levyAspxUrl: string;
...
export type CountyLevyStacksFile = {
  snapshot: {
    bundledAsOf: string;
    source: string;
    taxYear?: string | null;
  ...
  stacksByTagId: Record<string, CountyLevyStack>;
};

/** Per parcel from Main Parcel Table: TotalActual (market), TotalAssessed. */
export type CountyPinToTagRow = {
  tagId: string;
...
export type CountyPinToTagFile = {
  snapshot: {
    bundledAsOf: string;
    source: string;
    taxYear?: string | null;
   ...
   */
  pinDigits: number;
  byPin: Record<string, CountyPinToTagRow>;
};
...
export type CountyParcelRecordRow = {
  ain?: string | null;
...
export type CountyParcelRecordByPinFile = {
  snapshot: {
    bundledAsOf: string;
    source: string;
    taxYear?: string | null;
  ...
  shardPrefix?: string;
  byPin: Record<string, CountyParcelRecordRow>;
};
...
const ainToPinIndexCache = new WeakMap<CountyPinToTagFile, Map<string, string>>();

/**
 * Digits-only AIN → first PIN that carries that AIN (Main Parcel export).
 * Built once per loaded pin map. Pass the county that owns the file so public
 * parcel id length is not assumed Arapahoe.
 */
export function getAinToPinIndex(
  file: CountyPinToTagFile,
  config: CountyConfig,
): Map<string, string> {
  const cached = ainToPinIndexCache.get(file);
  if (cached) return cached;
  const idx = new Map<string, string>();
  const publicDigits = config.publicParcelId?.digits;
 ...
 * Pass the resolved county config (required).
 */
export function resolvePinKeyFromParcelIdInput(
  ...
  if (ainCands.length > 0) {
    const ainIndex = getAinToPinIndex(file, config);
...
>();
const pinCacheByRoot = new Map<string, Promise<CountyPinToTagFile | null>>();
...
function isCountyLevyStackLine(value: unknown): boolean {
...
function isCountyLevyStack(value: unknown): boolean {
  if (!isPlainObject(value)) return false;
  ...
  return value.lines.every(isCountyLevyStackLine);
}

function isCountyPinToTagRow(value: unknown): boolean {
  if (!isPlainObject(value)) return false;
 ...
 * `sourceUrl` is for error messages only (pass the fetch URL or fs path).
 */
export function validateCountyLevyStacksFile(
  data: unknown,
  sourceUrl: string,
): string | null {
 ...
 * `sourceUrl` is for error messages only (pass the fetch URL or fs path).
 */
export function validateCountyPinToTagFile(
  data: unknown,
  sourceUrl: string,
): string | null {
  if (!isPlainObject(data)) {
...
export function getLastCountyLevyStacksFetchFailureDetail(): string | null {
...
export function fetchCountyLevyStacksJson(
  ...
  const pending = (async () => {
    ...
    const invalidDetail = validateCountyLevyStacksFile(result.json, url);
    if (invalidDetail) {
    ...
    return result.json as CountyLevyStacksFile;
  })();
...
/**
 * Fetch `{countyId}-pin-to-tag.json` with validation and per-root cache.
 * URL: {@link countyAccountMapUrl} + optional `?v=` from {@link COUNTY_ACCOUNT_MAP_CACHE_BUST}.
 * `countyId` is required (resolved or campaign-default id passed explicitly).
 */
export function fetchCountyPinToTagJson(
  countyId: string,
  dataRoot?: string,
): Promise<CountyPinToTagFile | null> {
  const root = normalizeLoaderDataRoot(dataRoot);
  const id = countyIdForDataPaths(countyId);
  ...
  const url = countyAccountMapUrl(id, root, COUNTY_ACCOUNT_MAP_CACHE_BUST);
  const pending = (async () => {
    const result = await fetchCountyStaticJson(url);
    if (!result.ok) {
    ...
    const invalidDetail = validateCountyPinToTagFile(result.json, url);
    if (invalidDetail) {
    ...
    return result.json as CountyPinToTagFile;
  })();
...
const parcelRecordShardCache = new Map<
  string,
  Promise<CountyParcelRecordByPinFile | null>
>();
 ...
 * Unknown `countyId` yields no prefixes (does not fall back to Arapahoe).
 */
 ...
 */
function fetchCountyParcelRecordShard(
  countyId: string,
  prefix: string,
  dataRoot?: string,
): Promise<CountyParcelRecordByPinFile | null> {
 ...
 */
export async function fetchCountyParcelRecordForPin(
  pinInput: string,
  countyId: string,
  dataRoot?: string,
): Promise<{
  row: CountyParcelRecordRow;
  ...
  for (const prefix of prefixes) {
    const file = await fetchCountyParcelRecordShard(id, prefix, root);
    if (!file) continue;
...
export function lookupParcelRecordRow(
  pinInput: string,
  file: CountyParcelRecordByPinFile,
  countyId: string,
...
export function clearCountyParcelDataCache(): void {
