...
/**
 * Jump list. On `<lg`, {@link HomeDashboardSectionNav} sets an inline max-height from
 * the list top to the viewport bottom so a full curated list (Try demo property)
 * can scroll inside the sticky strip; without that, sticky pins the top and lower
 * taps never land (common on iOS Safari). `overscroll-contain` limits scroll chaining.
 */
