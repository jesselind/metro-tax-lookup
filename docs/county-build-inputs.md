...
| `supporting-data/dola/lg-export-all.csv` | no (directory rebuild) | [DOLA LGIS portal](https://dola.colorado.gov/dlg_lgis_ui_pu/) | — | `build:district-directory` (contacts filtered to LGIDs present in levy stacks) | **`arapahoe`**, **`douglas`** (union of stack LGIDs into one shared directory JSON) | Local only; **different export** from Property Tax Entities (mailing address / website / LGID). Not the mill / Tax Entity ID source. Broken/stale Website URL cells: curated `tools/district_directory_website_overrides.json` (LG ID → `https://…`), then rebuild + bump `SPECIAL_DISTRICT_DIRECTORY_CACHE_BUST`. |
...
**Also uses (Colorado shared):** `property-tax-entities-export.csv` (mill join; certifying county Douglas); `lg-export-all.csv` (optional, district directory rebuild — LGIDs unioned with Arapahoe stacks).
...
| `supporting-data/douglas/realware-detail/{stamp}/` (per-account JSON) | yes (Phase 15 valuation history) | Assessor SPA static files | `https://apps.douglas.co.us/realware/DATA/{stamp}/detail/{ACCOUNT}.json`; catalog `https://apps.douglas.co.us/realware/DATA/meta.json` | `douglas-valuation-history-by-account/` shards | **Build-time retain only** (gitignored). ~169k files / ~4 GB for 2026 stamp. Fetch: `npm run fetch:douglas-realware-detail` (uses `Property_Location.txt` account list). Stamp in tracked `tools/douglas-realware-detail-stamp.txt`. Extract/ship: `npm run build:douglas-valuation-history:ship` (or prove-out then `land:douglas`). Optional `--validate-meta-count` requires extracted accounts == `meta.json` `totalAccountsProcessed`; without it, a count mismatch is a warning (missing/unparseable retain files). After re-ship, bump `COUNTY_VALUATION_HISTORY_CACHE_BUST`. Do **not** live-fetch from the app. |
