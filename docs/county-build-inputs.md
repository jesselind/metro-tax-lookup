...
| `supporting-data/douglas/realware-detail/{stamp}/` (per-account JSON) | yes (Phase 15 valuation history) | Assessor SPA static files | `https://apps.douglas.co.us/realware/DATA/{stamp}/detail/{ACCOUNT}.json`; catalog `https://apps.douglas.co.us/realware/DATA/meta.json` | `douglas-valuation-history-by-account/` shards | **Build-time retain only** (gitignored). ~169k files / ~4 GB for 2026 stamp. Fetch: `npm run fetch:douglas-realware-detail` (uses `Property_Location.txt` account list). Stamp in tracked `tools/douglas-realware-detail-stamp.txt`. Extract: `npm run build:douglas-valuation-history` (full retain: add `--validate-meta-count` so extracted account count matches `meta.json` `totalAccountsProcessed`). Ship: `npm run build:douglas-valuation-history:ship` or `npm run land:douglas` after prove-out. Do **not** live-fetch from the app. |
