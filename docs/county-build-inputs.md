...
| `supporting-data/dola/lg-export-all.csv` | no (directory rebuild) | [DOLA LGIS portal](https://dola.colorado.gov/dlg_lgis_ui_pu/) | — | `build:district-directory` (contacts filtered to LGIDs present in levy stacks) | **`arapahoe`**, **`douglas`** (union of stack LGIDs into one shared directory JSON) | Local only; **different export** from Property Tax Entities (mailing address / website / LGID). Not the mill / Tax Entity ID source. |
