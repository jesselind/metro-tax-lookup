...
| **2. County data failures (gap chrome)** | We have (or tried) a source and the county export/hosting failed the taxpayer in a way we must name. | Opt-in gap flags on `features` + `knownFailures` for hosting failures on an existing source | `priorYearValuesGap: true` → always-visible prior-year COUNTY DATA GAP in Appraised / Assessed cards. `dataMartRefreshGap: true` → Assessor Data Mart incomplete refresh. `compsPdf` + `knownFailures.compsPdfHostedFiles` → comps COUNTY DATA GAP under Comparable properties. |
...
Builder: `safeCountyParcelRecordUrl(id, config, { year? })` in `src/lib/safeExternalHref.ts`. Lookup id: `countyParcelRecordLookupValue` (public parcel id when `publicParcelId` is set; otherwise account id). Douglas summary **Assessment year** / **Tax year** tiles and default `{year}` for hashPath links come from `resolveParcelSummaryYears` (`src/lib/parcelSummaryYears.ts`): maintainer stamp (`urls.parcelRecord.year`), Realware face tax year when valuation history is loaded (preferred over mill-PDF stack year), and levy-stack snapshot tax year as fallback — not invented from statute.

**`propertyTaxEstimateMode`** (on `CountyConfig`, not a feature flag): where the locked-report Property tax tile gets its dollar. `levyStackDollars` (default — same dual-base / single-base math as the mill-levy stack via `annualTaxDollarsForLevyStack`; Arapahoe), `realwareTaxDollars` (Douglas — Realware `taxDollars` + `alternateTaxDollars` for the face year; stack reconciles to face), or `omit`. Missing fields for the chosen mode → omit the dollar (fail closed). Never silently use a competing local × total mills face product. Never silently use mills math when the mode is `realwareTaxDollars`. Helper: `src/lib/propertyTaxEstimate.ts`. Agent contract: `.cursor/rules/property-tax-estimate-honesty.mdc`.

**Arapahoe Assessor UI (2026-09-15; face=stack 2026-09-17):** Parcel page shows **Assessed Value** and **Assessed School Value** plus a mill-levy total. **Tax District Levies** (`Levy.aspx`) lists Code / Taxing Authority / Levy (**mills only** — no per-line or footer dollar column). Classification **C3** (no county-published estimate dollars): face mode is `levyStackDollars` (app-derived; same as stack). Do not claim the face $ is a county-published total.

**Levy-line dollars (county-agnostic):** Mill levy tiles, stack total, and the Est. property tax face tile (under `levyStackDollars`) use `src/lib/levyLineAssessedBase.ts`. When school assessed is known and differs from local assessed, **school** authorities (authority-chain `family === "school"`, or a name match for `SCHOOL DIST` / `School District` / `School`/`Schools`) use school assessed; all other lines use local assessed. Stack **total** rounds each assessed side once (school mills sum × school assessed + local mills sum × local assessed) so it reconciles with Realware-style tax buckets; individual tiles still round per line and may differ from that total by a dollar or two. When school assessed is missing or equal to local, every line uses local (same as single-base). Face and stack must match under `levyStackDollars`.
...
| `propertyDataAccuracyWarning` | Amber **KNOWN ISSUE**: locked-report banner under Jump to… (scrolls away; not sticky) + `/sources` callout when shipped figures can disagree with the Assessor site. **Douglas off** after 5.15.0 Realware face tax + dual-rate levy dollars (incident copy modules retained for reuse). Arapahoe false. Reusable chrome: `KnownIssueBanner` / `KnownIssueCallout`; Douglas copy in `douglasPropertyDataAccuracyWarning.ts`. Not COUNTY DATA GAP and not sky Coming soon. |
...
| `priorYearValuesGap` | COUNTY DATA GAP: no free bulk prior-year assessed story for this county. **Dashboard:** always-visible callouts in Appraised / Assessed kind cards (Arapahoe copy: assessor guidance that the public site has no history). Mill-chart footer keeps **Prior $ missing** badge+popover. **`/sources`** adds methodology detail when the flag is on. Module: `countyPriorYearValuesGapNote.tsx`. Mutually exclusive with `priorYearValuesInProgress`. |
