...
**`propertyTaxEstimateMode`** (on `CountyConfig`, not a feature flag): where the locked-report Property tax tile gets its dollar. `singleAssessedTimesTotalMills` (Arapahoe), `realwareTaxDollars` (Douglas — Realware `taxDollars` + `alternateTaxDollars` for the face year), or `omit`. Missing fields for the chosen mode → omit the dollar (fail closed). Never silently use mills math when the mode is `realwareTaxDollars`. Helper: `src/lib/propertyTaxEstimate.ts`. Agent contract: `.cursor/rules/property-tax-estimate-honesty.mdc`.

**Arapahoe Assessor UI (Track B, 2026-09-15):** Parcel PPI shows **Assessed Value** and **Assessed School Value** plus a mill-levy total. **Tax District Levies** (`Levy.aspx`) lists Code / Taxing Authority / Levy (**mills only** — no per-line or footer dollar column). Classification **C3** (no county-published estimate dollars): keep Arapahoe face mode `singleAssessedTimesTotalMills`. Do not treat “matches county” as proof the county applies dual-base on a public estimate UI — they do not publish that estimate on Levy.aspx / PPI.

**Levy-line dollars (county-agnostic):** Mill levy tiles, stack total, and related dollar lines use `src/lib/levyLineAssessedBase.ts`. When school assessed is known and differs from local assessed, **school** authorities (authority-chain `family === "school"`, or a name match for `SCHOOL DIST` / `School District` / `School`/`Schools`) use school assessed; all other lines use local assessed. Stack **total** rounds each assessed side once (school mills sum × school assessed + local mills sum × local assessed) so it reconciles with Realware-style tax buckets; individual tiles still round per line and may differ from that total by a dollar or two. When school assessed is missing or equal to local, every line uses local (same as single-base). Face tile source (`propertyTaxEstimateMode`) stays a separate county config knob. On Arapahoe, face (single-base) and stack (dual-base) can diverge when school ≠ local; that is intentional after Track B C3.
