...
- Optional **authority chain** (sourced who authorized this mill): `public/data/levy-authority-chain-entries.json`, `LevyAuthorityChainSection`, `npm run validate:levy-authority-chain`. Shared chrome labels live in the component (`See each step`, `What we still cannot say`). Every fact needs an `https` source URL (`validate:levy-authority-chain` checks the scheme only; official-host provenance stays a manual check). Do **not** hard-code another parcel's TAG in district-wide entries. Parcel verification already lives below the levy tiles. Match helpers live in `src/lib/levyEntryMatch.ts` (same order as levy explainers: line code, LG ID + label, TAG, label keywords). Authority-chain label-only lookup skips entries that already declare a keyed match (`levyLineCode` / `lgId` / `sourceTagId`).
  - **Attribute reported outcomes:** do not present an election result as independently verified by this app. Start with plain attribution such as "According to Arapahoe County's certified election results..." and use `summarySource` to link that exact phrase to the mobile-friendly county results page.
  - **Define jargon in place, do not jump:** to gloss one word in a step `title` or `body`, set `titleTermId`/`titleTermMatch` or `bodyTermId`/`bodyTermMatch`. Allowed ids live in top-level JSON `allowedInlineTermIds` (same list the UI and validator use; currently `term-mill-levy`, `term-mills`). It renders a brief popover on that word, not a jump to the modal footer. The validator checks each id is in that list, resolves a flow glossary brief (`isFlowGlossaryTermId`), and that the word appears in the chosen field.
  - **Deep-link PDFs:** when a source PDF has a specific page, append `#page=N` to the URL and say so in the link text (e.g. `opens to page 21`). Cite the county **Notice of Election** (TABOR notice) for exact ballot wording, not a "sample ballot."
