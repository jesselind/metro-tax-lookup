...
**`millsReferenceCountyId`** on registry rows is **required**. It points at the county bundle used to validate curated AUTH history at build time and, when the resident county has no mills bundle, to supply **entity-level YoY numbers** for registry-linked stack lines (not rate-table links or modal charts). `resolveAuthorityMillsLookup` never falls back to Arapahoe when the resident county is missing.
