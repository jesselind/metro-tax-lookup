...
4. **Valuation history (Phase 15):** after Realware detail JSON is retained under `supporting-data/douglas/realware-detail/{stamp}/`, run `npm run build:douglas-valuation-history:ship` (or extract into `supporting-data/_ingest-out/douglas` and include shards in `land:douglas`). Optional `--validate-meta-count` for strict meta equality; without it, count mismatches warn. Bump `COUNTY_VALUATION_HISTORY_CACHE_BUST` after re-ship. Dashboard reads `douglas-valuation-history-by-account/` only; no live Douglas fetches.
