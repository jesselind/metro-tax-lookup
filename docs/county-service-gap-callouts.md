...
| --- | --- | --- |
| **Inline note** | Property details meta, Comparable properties COUNTY DATA GAP body | `<CountyServiceGapCallout density="compact">` + incident copy module |
| **Summary tile** | Legacy only. Prefer section homes after the levy-first layout (comps PDF chrome under Comparable properties heading; prior-year assessed as chip+popover on Appraised and assessed values). `COUNTY_SERVICE_GAP_SUMMARY_TILE_CLASS` tokens remain if a surface still needs a gap tile. | `COUNTY_SERVICE_GAP_SUMMARY_TILE_CLASS` + `<CountyServiceGapHeader>` + status line; may open a popover. Do **not** wrap `CountyServiceGapCallout` inside a tile grid cell. |
| **Chip status + popover** | Secondary missing fact on a still-valid value (prior-year assessed on Appraised and assessed values); mill-history chart footer when oldest year lacks assessed dollars **and** newest year has dollars; comps **Coming soon** under the Comparable properties heading | Red status badge (`CountyServiceGapBadge`) or sky `InProgressBadge`. Badge is the `InfoHintPopover` trigger. Gap: `variant="county-data-gap"`. Reuse `CountyPriorYearValuesGapPopover` on the levy modal mill chart when `priorYearValuesGap` is on. |
...
The prior-year values gap is a red **Prior years missing** badge on the **Assessed value** row in Appraised and assessed values, not a red summary tile and not a teaching modal. Do not paint that chip with `COUNTY_SERVICE_GAP_SUMMARY_TILE_CLASS`. The popover is the dashboard interrupt: same `InfoHintPopover` behavior as glossary briefs, `variant="county-data-gap"` for COUNTY DATA GAP chrome on the panel, header + copy inside. On the levy modal **Total mills** chart footer, use **Prior $ missing** for the same gap (mills timeline is present; prior-year dollar estimates are not). There is no separate Mill levy total summary chip; each levy tile keeps its own **Changed** when that line moved.
