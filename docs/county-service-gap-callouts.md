...
| **Chip status + popover** | Secondary missing fact on a still-valid chip (prior-year assessed); mill-history chart footer when oldest year lacks assessed dollars **and** newest year has dollars | Red status badge on the white chip (`CountyServiceGapBadge`: same layout as `LevyChangedBadge`, `COUNTY_SERVICE_GAP_BADGE_TONE_CLASS`). Badge is the `InfoHintPopover` trigger. Same popover as tile glossary briefs (`PARCEL_GLOSSARY_POPOVER_PANEL_CLASS` for width/scroll); `variant="county-data-gap"` paints the panel with `COUNTY_SERVICE_GAP_SURFACE_TONE_CLASS` (thin red border + light red fill). COUNTY DATA GAP header + copy sit inside that panel. Reuse `CountyPriorYearValuesGapPopover` on the levy modal mill chart when `priorYearValuesGap` is on (**Prior $ missing** on the chart; **Prior years missing** on Assessed value). Do **not** paint the chip with `COUNTY_SERVICE_GAP_SUMMARY_TILE_CLASS`. Do **not** nest `CountyServiceGapCallout` inside the panel. |
...
| County comps PDF hosting limited | `countyCompsPdfGapNote.tsx`, `countyCompsPdfGuidance.ts` | `features.compsPdf` + `knownFailures.compsPdfHostedFiles` | **Comparable properties** summary tile + popover (`#home-parcel-comps-pdf`). If `features.compsPdf` is false and `compsPdfInProgress` is false, omit the tile (no gap). | #county-comps-pdf-gap → under **Comparable properties** |
| Douglas comps PDF not wired yet | `countyCompsPdfInProgressNote.tsx` | `features.compsPdfInProgress` (Douglas on; Arapahoe false) | Sky **Coming soon** badge on **Comparable properties** (IN PROGRESS chrome, not COUNTY DATA GAP) | #county-comps-pdf-in-progress → Douglas **Property details** (sky callout; not the gap hub) |
| Douglas property figures known issue | `douglasPropertyDataAccuracyWarning.ts` + `KnownIssueBanner` / `KnownIssueCallout` | `features.propertyDataAccuracyWarning` (Douglas on; Arapahoe false) | Amber **KNOWN ISSUE** banner under Jump to… on the locked report (not sticky) | #county-property-data-accuracy-warning → Douglas account lookup (amber callout; not the gap hub) |
...
| Douglas valuation history | Assessed value / Actual value summary tile modals (single-series chart + table) | `features.valuationHistoryShards` (Douglas) | **Changed** badge on **Assessed value** / **Actual value** when prior year differs; tap tile or **View valuation history** in Property details | `/sources` Douglas Property details methodology (Realware detail JSON) |
